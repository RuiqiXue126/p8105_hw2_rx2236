...
The combined dataset has 471 unique id which means there are 471
participants appear in both datasets. This combined dataset has 8
variables which include all variables from the baseline and the amyloid
dataset, since the only shared variable in these two datasets is the
identifier `id`.
