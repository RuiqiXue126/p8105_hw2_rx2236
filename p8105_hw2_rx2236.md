...
Up to this point we cleaned the dataset with proper variable names, and
encoded the `sex` and `APOE4 carrier status` as non-numeric. From here,
we see that 483 patients were recruited at the baseline.
...
So after removing those participants, we get the number of participants
who developed MCI is 97.
